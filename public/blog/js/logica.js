...
function crearEntrada(titulo, contenido, etiquetas, imagenes) {
  const entrada = {
    id: crearIdUnico(),
    titulo: titulo.trim(),
  ...
  // Campos opcionales: solo se incluyen si hay, así el JSON exportado se queda
  // limpio para entradas simples.
  if (Array.isArray(etiquetas) && etiquetas.length > 0) entrada.etiquetas = etiquetas;
  if (Array.isArray(imagenes) && imagenes.length > 0) entrada.imagenes = imagenes;
  return entrada;
}
