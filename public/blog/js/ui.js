...
function leerFormularioEntrada() {
  return {
    ...
    etiquetas: leerListaDelCampo('campo-etiquetas', { minusculas: true, dedup: true }),
    imagenes: leerListaDelCampo('campo-imagenes', { minusculas: false, dedup: false }),
  };
}

// Campo opcional en el markup: null-safe. Devuelve array de strings limpias,
// con o sin normalización según lo que se pase. Reemplaza al viejo lector de
// etiquetas — misma forma, un solo lugar para tocar.
function leerListaDelCampo(id, opciones) {
  const campo = document.getElementById(id);
  if (!campo || !campo.value.trim()) return [];
  const sueltas = campo.value.split(',').map((t) => t.trim()).filter(Boolean);
  const normalizadas = opciones.minusculas ? sueltas.map((t) => t.toLowerCase()) : sueltas;
  return opciones.dedup ? [...new Set(normalizadas)] : normalizadas;
}

function limpiarFormularioEntrada() {
  campoTitulo.value = '';
  campoContenido.value = '';
  for (const id of ['campo-etiquetas', 'campo-imagenes']) {
